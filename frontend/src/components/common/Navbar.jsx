...
export default function Navbar() {
  ...
  const navLinks = [
    { name: 'Find Parking', path: '/home', icon: MapPin },
    { name: 'Current Pass', path: '/current-booking', icon: Clock },
          ...
          <div className="hidden md:flex items-center gap-3">
            {isAuthenticated ? (
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="px-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 hover:text-brand-600"
                >
                  Log In
                </Link>
                <Link
                  to="/register"
                  className="px-4 py-2 text-xs font-semibold bg-brand-600 hover:bg-brand-700 text-white rounded-xl shadow-md shadow-brand-600/20"
                >
                  Register
                </Link>
              </div>
        ...
        <div className="md:hidden border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 pt-2 pb-4 space-y-1 animate-in slide-in-from-top duration-200">
          ...
          {isAdmin && (
            <Link
              to="/admin"
              onClick={() => setMobileMenuOpen(false)}
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold bg-amber-500/10 text-amber-600 dark:text-amber-400 mt-2"
            >
