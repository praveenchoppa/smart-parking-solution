...
export default function ParkingCard({ parking, onSelect }) {
  ...
  const calculatedOccupied = occupiedSlots !== undefined ? occupiedSlots : Math.max(0, totalSlots - availableSlots);
  const occupancyInfo = getOccupancyStatus(occupancyPercentage);

  const latStr = latitude ? Number(latitude).toFixed(4) : 'N/A';
  const lngStr = longitude ? Number(longitude).toFixed(4) : 'N/A';
  ...
  return (
    <div className={`relative bg-white dark:bg-slate-900 border rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between ${
      recommendationAvailable ? 'border-brand-500 ring-2 ring-brand-500/20' : 'border-slate-200 dark:border-slate-800'
    }`}>
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div>
            ...
            <div className="mt-1.5 inline-flex items-center gap-1 text-[10px] font-mono text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md">
        ...
        {recommendationReason && (
          <div className="bg-brand-50 dark:bg-brand-950/50 border border-brand-200 dark:border-brand-800/40 rounded-xl p-2.5 mb-3 text-xs text-brand-800 dark:text-brand-300">
            <strong>Note:</strong> {recommendationReason}
          </div>
        )}

        {/* Distance & Slots breakdown */}
        <div className="grid grid-cols-3 gap-2 my-4 text-center">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 p-2 rounded-xl border border-slate-100 dark:border-slate-800">
            <span className="text-[9px] uppercase font-bold text-emerald-500 block">Available</span>
            <span className="text-xs font-black text-emerald-600 dark:text-emerald-400 mt-0.5 block">
              {availableSlots} / {totalSlots}
            </span>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/60 p-2 rounded-xl border border-slate-100 dark:border-slate-800">
            <span className="text-[9px] uppercase font-bold text-rose-500 block">Occupied</span>
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
        ...
        <a
          ...
          className="p-2 text-slate-500 hover:text-brand-600 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors shrink-0"
        ...
        {onSelect && (
          <Button onClick={() => onSelect(id)} variant="outline" size="sm">
            Select
          </Button>
        )}
      </div>
